const MainInput = ({ label, type, value, setValue, placeholder, options }) => {
    return (
        <label className="w-full mb-[20px] block">
            ...
            {type !== 'textarea' && type === 'select' && <select required placeholder={placeholder} className="w-full bg-white shadow-md focus:outline-none outline-none p-5 rounded-[10px] cursor-pointer" value={value} onChange={(e) => setValue(e.target.value)}>
                {options.map(e => <option key={e._id} value={e._id}>{e.title}</option>)}
            </select>}
